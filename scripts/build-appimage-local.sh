...
APPIMAGE_NAME="${APPIMAGE_NAME:-Seraph-${VERSION_LABEL}-x86_64.AppImage}"
...
cmake -S "$ROOT_DIR" -B "$BUILD_DIR" "${GENERATOR_ARGS[@]}" \
    ...
    -DSERAPH_ENABLE_QML_CACHEGEN=ON \
    -DBUILD_TESTS=OFF
...
"$LINUXDEPLOY" \
    --appdir "$APPDIR" \
    --plugin qt \
    --desktop-file "$ROOT_DIR/dist/io.github.raphamzn.Seraph.desktop" \
    --icon-file "$ROOT_DIR/dist/io.github.raphamzn.Seraph.svg"
