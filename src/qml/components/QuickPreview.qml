...
Item {
    ...
    // Markdown gets rendered (headings, bold, lists) instead of shown as raw
    // source. Qt's TextEdit renders CommonMark natively via
    // TextEdit.MarkdownText, so no external tool is involved.
    readonly property bool isMarkdown: {
        if (!isText)
            return false
        if (_mime === "text/markdown" || _mime === "text/x-markdown")
            return true
        return ["md", "markdown", "mdown", "mkd", "mkdn", "mdwn"].indexOf(fileExtension) >= 0
    }
                    ...
                    Item {
                        ...
                        Column {
                            ...
                            Image {
                                ...
                                width: 64
                                height: 64
                                source: "image://icon/" + (root.fileProps.iconName || (root.isPdf ? "application-pdf" : "image-x-generic")) + Theme.iconQuery
                                sourceSize: Qt.size(width, height)
                                fillMode: Image.PreserveAspectFit
                            }
                        ...
                        Column {
                            ...
                            Image {
                                ...
                                width: 96
                                height: 96
                                source: "image://icon/" + (root.fileProps.iconName || "application-pdf") + Theme.iconQuery
                                sourceSize: Qt.size(width, height)
                                fillMode: Image.PreserveAspectFit
                            }
                        ...
                        Column {
                            ...
                            Image {
                                ...
                                width: 96
                                height: 96
                                source: "image://icon/" + (root.fileProps.iconName || "text-x-generic") + Theme.iconQuery
                                sourceSize: Qt.size(width, height)
                                fillMode: Image.PreserveAspectFit
                            }
